...
namespace HASB_Doctor.Controllers
{
    ...
    public class OperationController : BaseDoctorController
    {
        ...
        [SwaggerOperation(Summary = "Lấy các xét nghiệm để yêu cầu bệnh nhân xét nghiệm, search cho operation tính sau (giả)")]
        [HttpGet]
        public async Task<IActionResult> GetOperations([FromQuery] long roomId)
        {
            try
            {
